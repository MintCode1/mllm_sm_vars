...
class DefaultModelState(ModelState):
    def __init__(
        ...
        # Reference to metrics tracker (set by the runner)
        self.metrics = None

    def set_metrics(self, metrics):
        """Set the metrics tracker reference for recording encoder timing."""
    ...
    def get_mm_embeddings(
        ...
        if mm_kwargs:
            # Execute the multimodal encoder with timing tracking
            enc_start = time.time()
            encoder_outputs = self.encoder_runner.execute_mm_encoder(mm_kwargs)
            enc_end = time.time()

            # Record encoder timing if metrics tracker is available
            if self.metrics is not None:
                self.metrics.record_phase("encoder", enc_end - enc_start)
