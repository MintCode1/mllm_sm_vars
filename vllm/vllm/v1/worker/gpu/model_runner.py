...
class GPUModelRunner(LoRAModelRunnerMixin):
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        # Metrics tracking for controlled experiments.
    ...
    def load_model(self, load_dummy_weights: bool = False, *args, **kwargs) -> None:
        ...
        # Pass metrics reference to model_state for encoder timing tracking
        if hasattr(self.model_state, 'set_metrics'):
            self.model_state.set_metrics(self.metrics)
    ...
    def execute_model(
        ...
        if not dummy_run:
            ...
            if scheduler_output.total_num_scheduled_tokens == 0:
                ...
                # Record request latency even for empty requests
                request_end = time.time()
                self.metrics.record_request_latency(request_end - request_start)
                return empty_output
        ...
        if hasattr(scheduler_output, "total_num_scheduled_tokens"):
            self.metrics.add_tokens(scheduler_output.total_num_scheduled_tokens)
        ...
        if batch_desc.num_tokens == 0:
            ...
            empty_output = self.kv_connector.no_forward(scheduler_output)
            # Record request latency even for empty batches
            request_end = time.time()
            self.metrics.record_request_latency(request_end - request_start)
            return empty_output
        ...
        # Determine phase (prefill or decode) for timing purposes
        ...
        phase_end = time.time()
        self.metrics.record_phase(phase, phase_end - phase_start)
        ...
        # Record request latency for last PP rank
        request_end = time.time()
        self.metrics.record_request_latency(request_end - request_start)
        return None
